...
obj_cut_defs = {
    ...
    "mu_ljs": {
        ...
        ">2 mulj": lambda objs: (ak.num(objs["mu_ljs"]>1) & ak.num(objs["egm_ljs"]==0)),
    },
    "genMus":{
...
evt_cut_defs = {
    ...
    ">=2 mu_ljs": lambda objs: ak.num(objs["mu_ljs"]) >= 2, #jsc
    ">1 muLjs >1 egmLJs": lambda objs: (ak.num(objs["mu_ljs"]) >= 1) & (ak.num(objs["egm_ljs"]) >= 1), #jsc
    ...
    "dPhi(LJ_0, LJ_1) > 1.8": lambda objs: abs(objs["ljs"][:, 0].delta_phi(objs["ljs"][:, 1])) > 1.8,
    "dPhi(LJ_0, LJ_1) > 2": lambda objs: abs(objs["ljs"][:, 0].delta_phi(objs["ljs"][:, 1])) > 2.0,
    "dPhi(LJ_0, LJ_1) > 2.2": lambda objs: abs(objs["ljs"][:, 0].delta_phi(objs["ljs"][:, 1])) > 2.2,
}
